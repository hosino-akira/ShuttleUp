...
const options = computed(() => {
  ...
  if (
    name &&
    !props.opponents.some((item) => item.name === name)
  )
    items.push({
      value: -1,
      label: `＋「${name}」を新規登録`,
  ...
  try {
    if (action.value === "delete") {
      ...
      if (props.mode === "edit" && props.match)
        await updateMatch(props.match.id, request);
      ...
      <a-row :gutter="16">
            ...
            :rules="[
              {
                ...
                type: 'number',
                min: 1,
                message: '対戦相手を選択してください。',
              },
            ]"
          >
            <a-select
              v-model:value="form.opponentId"
              show-search
              option-filter-prop="label"
              :options="options"
              placeholder="名前で検索"
              @search="opponentSearch = $event"
              @change="handleOpponentChange"
            />
            <a-button
              type="link"
              class="manage-button"
              @click="emit('manageOpponents')"
              >対戦相手を管理</a-button
            >
          </a-form-item></a-col
        >
        ...
        <a-col :xs="12" :md="6"
          ><a-form-item label="自分のスコア"
            ><a-input-number
              v-model:value="form.myScore"
              :min="0"
              class="full-width" /></a-form-item
        ></a-col>
        <a-col :xs="12" :md="6"
          ><a-form-item label="相手のスコア"
            ><a-input-number
              v-model:value="form.opponentScore"
              :min="0"
              class="full-width" /></a-form-item
        ></a-col>
        <a-col :xs="24" :md="12"
          ><a-form-item label="動画URL"
            ><a-input
              v-model:value="form.videoUrl"
              :maxlength="2048" /></a-form-item
        ></a-col>
      </a-row>
